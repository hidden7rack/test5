...
var db = new mongodb.Db("dataB8", mongodbServer);
var usersssssss="";
    ...
    if (request.method == "POST") {
	   ...
	   if(obj['act']=="signup"){
				//if (obj.signup != null) {
					...
					console.log("SIGNUP");
					// Send obj data to dataB8
			 
					db.open(function() {
						
						db.collection("user2", function(err, collection) {
...
//	response.end('{"success" : "Updated Successfully", "status" : 200}');
		...
		console.log("input pass="+obj.pw);
									 ...
									 MongoClient.connect("mongodb://localhost:27017/dataB8", function (err, db) {
						db.collection("user2", function (err, collection) {
							collection.find().toArray(function(err, items) {
								if(err) throw err;
								// Check whether there is data in the dataB8
